...
export interface TuiState {
  ...
  todos: TodoItem[]
  items: ChatItem[]
  /** Last error message shown to the user (cleared on the next user submit). */
  error: string
...
export function reduce(state: TuiState, event: TuiEvent): TuiState {
  switch (event.type) {
    ...
    case 'thinking-delta':
      return patchAssistant(state, event.id, item => ({ ...item, thinking: item.thinking + event.text, pending: true }))
    ...
    case 'turn-start':
      return { ...state, turn: event.turn, step: 0, phase: 'working' }
    case 'turn-end':
